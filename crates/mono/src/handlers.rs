...
pub fn is_localhost(host: &str) -> bool {
    let host = host.to_lowercase();
    host == CONFIG.get_localhost_port() || host == CONFIG.get_127_port()
}
...
mod tests {
    ...
    #[test]
    fn test_is_host_exact_match_only() {
        // "kominick.com" should NOT match "sub.kominick.com"
        assert!(!is_host("sub.kominick.com", &["kominick.com"]));
    }

    #[test]
    fn test_is_host_multiple_targets() {
        assert!(is_host(
            "james.kominick.com",
            &["kominick.com", "james.kominick.com"]
        ));
        assert!(is_host("kominick.org", &["kominick.com", "kominick.org"]));
        assert!(!is_host("other.com", &["kominick.com", "kominick.org"]));
    }

    #[test]
    fn test_is_host_empty_targets() {
        // Without localhost fallback in is_host, these should NOT match
        assert!(!is_host("localhost:3003", &[]));
        assert!(!is_host("127.0.0.1:3003", &[]));
        assert!(!is_host("example.com", &[]));
    }

    #[test]
    fn test_is_host_mixed_case_target() {
        // Host is lowercased before comparison
        assert!(is_host("Kominick.COM", &["kominick.com"]));
        assert!(is_host("SPOTIE.APP", &["spotie.app"]));
    }

    #[test]
    fn test_is_host_git_domain() {
        assert!(is_host("git.jaemk.me", &["git.jaemk.me"]));
        assert!(!is_host("jaemk.me", &["git.jaemk.me"]));
    }
}
