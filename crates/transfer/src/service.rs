    ...
    State: axum::extract::FromRef<S>,
{
    Router::new()
        ...
        .route("/api/download/confirm", post(handlers::api_download_confirm))
        // Auth API
        .route("/api/auth/register",    post(handlers::api_auth_register))
        .route("/api/auth/verify-code", post(handlers::api_auth_verify_code))
        .route("/api/auth/resend-code", post(handlers::api_auth_resend_code))
        ...
        // Settings API
        .route("/api/settings/add-password", post(handlers::api_settings_add_password))
        // Static assets
        ...
        .route_service("/register", ServeFile::new("crates/transfer/web/register.html"))
        .route_service("/my",       ServeFile::new("crates/transfer/web/my.html"))
        .route_service("/settings", ServeFile::new("crates/transfer/web/settings.html"))
        // Existing pages
        .route_service("/download", ServeFile::new("crates/transfer/web/download.html"))
...
pub async fn init(config: crate::Config) -> anyhow::Result<State> {
    ...
    let state = std::sync::Arc::new(Resources { db, s3, http, config });
